...
import { getClosedTickets, getMyTickets } from "@/services/tickets";
...
export default function CloseTickets() {
  ...
  async function loadTickets() {
    ...
    try {
      ...
      if (user?.roll === "admin" || user?.roll === "assistance") {
        data = await getClosedTickets();
      } else {
        const allTickets = await getMyTickets();
        data = allTickets.filter((t: any) => t.status === "closed");
      }
  ...
  async function handleExport() {
    try {
      ...
      exportTicketsToExcel(enrichedTickets, 'tickets_cerrados');
    ...
    <div className="space-y-6 w-full max-w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Revisa el historial de tickets resueltos
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded-lg px-4 py-2">
            <span className="text-sm text-gray-600 dark:text-gray-300">
              Total: <span className="font-bold text-green-600 dark:text-green-400">{filteredTickets.length}</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 dark:focus:ring-green-600 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-400"
          />
        </div>
      </div>
      ...
      {filteredTickets.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 p-12 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 text-center">
          <FiCheckCircle className="w-20 h-20 text-gray-400 dark:text-gray-500 mx-auto mb-4" />
          <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-2">
            {searchQuery ? "No se encontraron tickets" : "No hay tickets cerrados"}
          </h3>
          <p className="text-gray-500 dark:text-gray-400">
            {searchQuery 
              ? "Intenta con otros términos de búsqueda"
              : "Los tickets cerrados aparecerán aquí cuando se resuelvan"}
          </p>
        </div>
      ) : (
              ...
              <div className="p-6">
                <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-4 mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold border bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 border-green-300 dark:border-green-600">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          Cerrado: {new Date(ticket.updated_at || ticket.created_at).toLocaleDateString("es-ES", {
                            year: "numeric",
                            month: "long",
                            day: "numeric",
                            hour: "2-digit",
                  ...
                  <div className="lg:flex-shrink-0">
                    <Link
                      href={`/support-center/close-tickets/${ticket.id}`}
                      className="inline-flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white font-semibold px-6 py-3 rounded-lg transition-colors shadow-sm hover:shadow-md"
                    >
